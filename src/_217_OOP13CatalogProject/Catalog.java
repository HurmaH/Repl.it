package _217_OOP13CatalogProject;
...
public class Catalog {
	...
	public void loadItems() {
		...
		items.addAll(Arrays.asList("iPhone 6s", "iPhone 6s Plus", "iPhone X","MacbookPro", "ThumbDrive", "Beats HeadPhones", "Mouse", "Charger", "iPad", "Dyson Vacuum", "TV", "Apple Watch"));
	...
	public ArrayList<String> getItemsLessThanAMonthlyPrice(double price) {
		...
		for (int i=0; i<monthlyPayments.size(); i++) {
			if(monthlyPayments.get(i)<=price) {
				lessThan.add(items.get(i)+"-"+prices.get(i)+"-"+monthlyPayments.get(i));
			}
		}
		return lessThan;
	}
